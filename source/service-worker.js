...
(() => {
  /** 缓存库名称 */
  const CACHE_NAME = "ShinRayTechCache";
  /** 版本名称存储地址（必须以`/`结尾） */
    ...
    switch (data) {
      ...
      default:
        ...
        deleteCache(list).then(() => {
          if (data === "refresh") event.source.postMessage({ type: "refresh" });
        });
        break;
    }
  });
  ...
  function updateJson() {
    ...
    const parseChange = (list, elements, version) => {
      for (let element of elements) {
        ...
        if (jsonList) {
          for (let it of jsonList) list.push(new CacheChangeExpression(it));
        }
      }
      ...
      new Promise((resolve) => {
        ...
        const dbVersion = {
          write: (id) =>
            new Promise((resolve, reject) => {
              caches
                ...
                .then(function (cache) {
                  cache
                    .put(new Request(VERSION_PATH), new Response(id))
                    .then(() => resolve());
                })
                .catch(() => reject());
            }),
          read: () =>
            new Promise((resolve) => {
              caches
                .match(new Request(VERSION_PATH))
        ...
        dbVersion.read().then((oldData) => {
          ...
          if (!oldVersion) {
            dbVersion.write(
              `{"global":${global},"local":"${newVersion.local}"}`
            );
    ...
    return new Promise((resolve) =>
      fetchNoCache(url).then((response) =>
  ...
  class CacheChangeExpression {
    constructor(json) {
      ...
      switch (json["flag"]) {
        ...
        case "new":
          this.match = (url) =>
            url.endsWith("postsInfo.json") || url.match(/\/archives\//);
          break;
        case "page":
          this.match = (url) =>
            forEachValues((value) => url.match(new RegExp(`\/${value}(\/|)$`)));
          break;
        case "str":
          this.match = (url) => forEachValues((value) => url.includes(value));
          break;
        default:
          throw `未知表达式：${JSON.stringify(json)}`;
      }
    }
  }
})();
